...
#### **Longest Chain Rule:**
In bitcoin there is no TTP (trusted third party) to decide which transactions are valid, so nodes follow the chain that has the most cumulative ”Proof of Work”(PoW).
...
A fork can also exist by an attacker who tries to double spend. The owner generates 2 or more transactions which leads to the forks in the chain. The same principle still holds that is mention above. 
...
*   In Ethereum we have **Proof-of-Stake** instead of Proof-of-Work
		This will work with a consensus via capital risk. Validators are selected based on locked stake, but if they validate malicious blocks they loss there stake (slashing) which leads to exclusion of future consensus 
